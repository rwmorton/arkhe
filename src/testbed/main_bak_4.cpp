...
protected:
	...
	void setView()
	{
		...
		//if(persp) glMultMatrixf(Matrix44::perspective(m_camera.fov,m_camera.a,m_camera.n,m_camera.f));
...
int main(int argc,char **argv)
{
	...
	try
	{
		...
		std::cout << "TESTING REDUCTION FORUMULAS:" << std::endl;
		arkhe::foundation::Timer timer;
		const int N = 100000000;
		float val;
		timer.start();
		for(int i=0; i<N; i++)
		{
			val = i/2 - 1/2*Mathf::cos((float)i)*Mathf::sin((float)i);
		}
		timer.stop();
		std::cout << "NOT OPTIMIZED TOOK: " << timer.elapsed(Timer::SECONDS) << " seconds" << std::endl;
		...
		{
			//x/2 - 1/4 Sin[2 x]
			val = i/2 - Mathf::cos((float)i*2) * (1/4);
		}
		timer.stop();
		std::cout << "OPTIMIZED TOOK: " << timer.elapsed(Timer::SECONDS) << " seconds" << std::endl;
